"""nihonez.com 에서 JLPT N2 reading + listening 크롤링.

기반: jlpt 레포의 scripts/scrape-listening.py (N1) 패턴을 N2 URL 로 어댑트.

수행 단계
1. 11개 N2 시험 페이지에서 reading 문제 + passages 추출
...
사용
    ...
    python3 scripts/scrape-n2.py            # 11개 전부
    python3 scripts/scrape-n2.py n2_2025-07 # 특정 회차만
...
def parse_reading(html: str, test_data: dict) -> tuple[list, dict]:
    """
    nihonez N2 페이지에서 reading 문제 + passages 추출.

    N2 의 testData.sections[0]['subsections'] 구조를 분석 후 채워야 함.
